...
def cc():
        ...
        #cc_test_results.execute("select s.event_id,e.user_id,u.username,e.problem_id,p.course_id,p.testname from cc_events e,cc_users u,cc_submission_receipts s,cc_problems p where s.event_id=e.id and u.id=e.user_id and e.problem_id=p.problem_id order by s.event_id desc limit 1;");
        #cc_test_results.execute("select s.event_id,e.user_id,u.username,e.problem_id,p.course_id,p.testname,cc.name,cc.title,u.email,t.name,cc.year, s.num_tests_attempted,s.num_tests_passed from cc_events e,cc_users u,cc_submission_receipts s,cc_courses cc,cc_problems p,cc_terms t where cc.id =p.course_id and s.event_id=e.id and u.id=e.user_id and e.problem_id=p.problem_id and t.id=cc.term_id and left(e.timestamp,10) >= 1453401000 and left(e.timestamp,10) <=1453055400 and left(u.id,5) = '14ITA' order by u.id,p.problem_id ;");
...
def html_call():
        ...
        file_name = "cloudcoder_"+str(today)+"."+"txt"
...
subject = "CloudCoder Ebox Report on " + str(today)
#emails = ["lanitha.b@kgkite.ac.in"]
emails = ["pramyaen@gmail.com"]
#cc_emails = ["lifebalance@gmail.com","pramyaen@gmail.com"]
cc_emails = ["pramyaen@gmail.com"]
bcc_emails = "cloudcoder-noreply@kggroup.com"
#send_email(subject,body_text,bcc_emails, emails, cc_emails)
